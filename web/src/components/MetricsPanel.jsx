...
const MetricsPanel = ({ metrics }) => {
    ...
    // 辅助格式化函数，处理 undefined/null 并允许 0 值
    const fmt = (val) => (val !== undefined && val !== null) ? parseFloat(val).toFixed(2) : '0.00';
    const fmtPct = (val) => (val !== undefined && val !== null) ? `${parseFloat(val).toFixed(2)}%` : '0.00%';

    const data = [
      { key: '1', metric: '最终权益', value: fmt(metrics.final_value), icon: <WalletOutlined />, color: '#722ed1', bg: '#f9f0ff' },
      { key: '2', metric: '净利润', value: fmt(metrics.net_profit), icon: <AccountBookOutlined />, color: metrics.net_profit > 0 ? '#cf1322' : '#3f8600', bg: '#fff1f0', isPnl: true },
      ...
      { key: '9', metric: '一手最终赚钱数', value: fmt(metrics.one_hand_net_profit), icon: <TransactionOutlined />, color: '#13c2c2', bg: '#e6fffb', isPnl: true },
      ...
      { key: '12', metric: '一手盈利百分数', value: fmtPct(metrics.one_hand_profit_pct), icon: <PercentageOutlined />, color: '#eb2f96', bg: '#fff0f6', isPnl: true },
    ];
